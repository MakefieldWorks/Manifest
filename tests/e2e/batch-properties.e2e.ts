...
test('multi-selects a range, previews a mixed-value batch, and undoes it as one operation', async ({ appPage, electronApp, workspaceDir }) => {
  ...
  await addProperty(appPage, 'Device A', 'firmware', 'v3')
  ...
  await expect(appPage.getByRole('button', { name: 'Keep only Device C' })).toBeVisible()
  await c.click({ modifiers: [...toggle] })
  await expect(appPage.getByRole('button', { name: 'Keep only Device A' })).toBeVisible()
  await c.click({ modifiers: ['Shift'] })
  ...
  await expect(appPage.getByTestId('batch-preview')).toContainText('2 of 3 nodes will change')
  await appPage.screenshot({ path: test.info().outputPath('batch-property-dialog.png') })
  await expect(appPage.getByTestId('batch-preview')).toContainText('Device B')
  ...
  await expect(appPage.getByTestId('toast')).toHaveText('Updated 2 selected nodes')
...
test('supports keyboard range selection and Space toggling', async ({ appPage, electronApp, workspaceDir }) => {
  await createLab(appPage, electronApp, workspaceDir)
  await addRootChild(appPage, electronApp, 'Device A')
  await addRootChild(appPage, electronApp, 'Device B')
  await addRootChild(appPage, electronApp, 'Device C')
